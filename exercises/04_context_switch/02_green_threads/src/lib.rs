...
impl Scheduler {
    ...
    fn schedule_next(&mut self) {
        ...
        if let Some(next_idx) = next {
            ...
            if self.threads[prev_idx].state != ThreadState::Finished {
                self.threads[prev_idx].state = ThreadState::Ready;
            }
            let next_thread = &mut self.threads[next_idx];
            next_thread.state = ThreadState::Running;
            if let Some(entry) = next_thread.entry.take() {
                unsafe { CURRENT_THREAD_ENTRY = Some(entry) };
            }
            unsafe {
                let prev_ctx = &mut self.threads[prev_idx].ctx as *mut _;
                let next_ctx = &self.threads[next_idx].ctx as *const _;
